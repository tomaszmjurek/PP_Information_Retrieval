...
class LIFO_Policy:
    ...
    def updateURLs(self, c, retrievedURLs, retrievedURLsWD, iteration):
        sorted_urls = list(retrievedURLs)
        sorted_urls.sort(key=lambda url: url[len(url) - url[::-1].index('/'):])
        self.queue.extend(sorted_urls)
...
class FIFO_Policy:
    ...
    def getURL(self, c, iteration):
        if len(self.queue) == 0:
            self.queue = c.seedURLs.copy()  # refilled after finishing
            # return None
...
class LIFO_Cycle_Policy:
    ...
    def getURL(self, c, iteration):
        if len(self.queue) == 0:
            # self.restartPolicy(self, c)
            self.queue = c.seedURLs.copy()
            self.fetched.clear()
        #  removing cycles: if next element was visited it's skipped
        if self.queue[-1] in self.fetched:
            self.queue.pop()
            if len(self.queue) == 0:
                self.queue = c.seedURLs.copy()
                self.fetched.clear()
        selected = self.queue.pop()
...
class LIFO_Cycle_Policy:
    ...
    def updateURLs(self, c, retrievedURLs, retrievedURLsWD, iteration):
        # list must be sorted before being added
        sorted_urls = list(retrievedURLs)
        sorted_urls.sort(key=lambda url: url[len(url) - url[::-1].index('/'):])
        self.queue.extend(sorted_urls)
...
class LIFO_Authority_Policy:
    ...
    def computeAuthorityLevel(self, c):
        for url in c.URLs:
            
        authority_level = len(c.incomingURLs) + 1
...
class Container:
    def __init__(self):
        ...
        self.crawlerName = "IRbot"  # nazwa bota do pokazania
        # Example ID
        self.example = "exercise3"  # zmieniac na 2 i 3 do zadan
        # Root (host) page	- napisane zeby nie wychodzil poza katalog, prefix porownywany z uzyskanym
        ...
        self.generatePolicy = LIFO_Cycle_Policy(self.seedURLs)
        # Page (URL) to be fetched next		-next page to visit
        self.toFetch = None
...
def getNormalisedURLs(retrievedURLs):
    return set([x.lower() for x in retrievedURLs])
...
def getFilteredURLs(c, retrievedURLs):
    ...
    toLeft = set([url for url in toLeft if url.lower() != c.toFetch.lower()])
    if c.debug:
